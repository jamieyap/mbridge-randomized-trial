...
colnames(tab_summary_stats_H1) <- c("Decision Point", "No. Participants Included in Analysis")
```
...
colnames(tab_summary_stats_H2) <- c("Decision Point", "No. Participants Randomized")
```
...
# Collect results into a data frame
dat_exp_results_H2 <- data.frame(exp_estimates = diag(U),
                                 rrLB95 = rrLB95,
                                 rrUB95 = rrUB95)
dat_exp_results_H2 <- round(dat_exp_results_H2, digits = 3)
colnames(dat_exp_results_H2) <- c("est","95%CI: LB","95%CI: UB")
row.names(dat_exp_results_H2) <- c("exp(beta0+beta1)","exp(beta0+beta2)","exp(beta0+beta3)")
```
...
kable(dat_exp_results_H2, format="latex", booktabs=TRUE) %>% 
  kable_styling(latex_options = c("HOLD_position"))
```
